...
1. Run `bd ready --json` to get the list of ready (unblocked) issues.
...
3. Mark it in progress: `bd update <id> --status=in_progress --json`
4. Show issue details: `bd show <id> --json`
5. Implement the work. Follow normal development practices (tests,
   linting, commits) but stay focused and move fast.
6. When done, close it: `bd close <id> --json`
